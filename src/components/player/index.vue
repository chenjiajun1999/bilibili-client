...
import VolumeBar from "./PlayerVolumeBar.vue";
import ProgressBar from "./PlayerProgressBar.vue";
import PlayerBarrageScreen from "./PlayerBarrageScreen.vue";
...
const volumePercent = computed(() => {
	if (currentVolume.value < 0.005) return Math.ceil(currentVolume.value * 100);
	else return Math.round(currentVolume.value * 100);
});

const areaDom = ref();
const videoDom = ref(); //视频dom
const duration = ref(0); // 视频全长
const buffered = ref<TimeRanges>(); // 缓冲(TimeRanges)
const isShowCover = ref(true); // 是否显示封面
const isShowBarrage = ref(true); // 是否显示弹幕
const isFullscreen = ref(false); // 是否处于全屏模式
const isPageFullscreen = ref(false); // 是否处于网页全屏模式
const isPictureInPicture = ref(false); // 是否处于画中画模式
const pictureInPictureEnabled = ref(false); // 浏览器是否支持画中画模式
const isCursorStatic = ref(false); // 鼠标是否长时间静止不动
const isDraggingProgressBar = ref(false); // 是否处于拖动进度条的过程中
const isPlaying = ref(false); // 是否正在播放
const isShowLoading = ref(false); // 是否显示加载框
const isShowVolumeHint = ref(false); // 是否显示音量提示条（键盘触发）
const volumeHintTimeout = ref(0); // 音量提示条多久ms后隐藏
const controlsHintTimeout = ref(0); // 控制面板多久ms后隐藏
const currentSpeed = ref(1.0); // 当前倍速
const currentVolume = ref(1); // 当前音量（0-1），同时作用于当前音量条的长度；currentVolumeCache为该值的缓存
const currentVolumeCache = ref<number>();
const currentTime = ref(0); // 当前播放进度，仅作用于进度条UI，变更该项并不会作用于video.currentTime
const currentTimeLabel = ref("00:00"); // 当前播放进度的文字
const fullTimeLabel = ref("00:00"); // 视频总长度的文字
const barrageTimelineStart = ref(0); // 弹幕时间轴的起始时间点（手动调整进度条触发更新）
...
function updateProgressText() {
	currentTimeLabel.value = secondTimeFormat(videoDom.value!.currentTime);
	fullTimeLabel.value = secondTimeFormat(videoDom.value!.duration);
}
// 更新视频的当前播放时间，同时更新其他依赖
function updateVideoCurrentTime(newCurrentTime: number) {
	videoDom.value!.currentTime = newCurrentTime;
	barrageTimelineStart.value = newCurrentTime;
 ...
 * 前进视频播放进度
 */
function forwardCurrentTime() {
	updateVideoCurrentTime(videoDom.value!.currentTime + 5);
}
/* 后退视频播放进度
 */
function backwardCurrentTime() {
	updateVideoCurrentTime(videoDom.value!.currentTime - 5);
}
/* 拖动进度条
 */
function onDraggingProgress(value: number) {
 ...
 * 松开/释放拖动的进度条
 */
function onReleasedProgress(value: number) {
...
function increaseVolume() {
	isShowVolumeHint.value = true;
	volumeHintTimeout.value = 2000;
	const nowVolume = videoDom.value!.volume;
	if (nowVolume >= 0.9) {
		videoDom.value!.volume = 1;
		currentVolume.value = 1;
	} else {
		const newVolume = videoDom.value!.volume + 0.1;
		videoDom.value!.volume = newVolume;
...
function changeSpeed(e: MouseEvent) {
	...
	if (!(e.currentTarget instanceof HTMLButtonElement)) {
		throw new Error("changeSpeed: event target not an HTMLButtonElement");
	}
	// 获取选择的倍速
	const value = e.currentTarget.dataset.value;
	// 应用视频倍速
	videoDom.value!.playbackRate = Number(value);
	// 标记变更后的倍速，用于显示文字
	currentSpeed.value = Number(value);
}
/* 点击音量条后更新音量（value范围：0-1）
 */
function updateVolumeByClickBar(value: number) {
	if (videoDom.value!.muted && value) {
		videoDom.value!.muted = false;
	}
	videoDom.value!.volume = value;
...
function toggleShowBarrage() {
	isShowBarrage.value = !isShowBarrage.value;
}
/**
 * 切换静音模式
 */
function toggleMute() {
	if (videoDom.value!.muted) {
		videoDom.value!.muted = false;
		updateVolumeByClickBar(currentVolumeCache.value ?? 0.1);
	} else if (videoDom.value!.volume == 0) {
		updateVolumeByClickBar(0.1);
	} else {
		videoDom.value!.muted = true;
		currentVolumeCache.value = currentVolume.value;
		updateVolumeByClickBar(0);
	}
}
/**
 * 切换画中画模式
...
function togglePictureInPicture() {
	try {
		...
		flag ? videoDom.value!.requestPictureInPicture() : document.exitPictureInPicture();
		isPictureInPicture.value = flag;
	} catch (error) {
		console.error(error);
	}
}
...
onMounted(() => {
	videoDom.value.focus({ preventScroll: true });
	pictureInPictureEnabled.value = document.pictureInPictureEnabled && !videoDom.value.disablePictureInPicture;
	let interval = function () {
		// 定时更新进度条
		if (isPlaying.value && !isDraggingProgressBar.value) {
			currentTime.value = videoDom.value!.currentTime;
		}
		// 定时更新进度的文字显示
		updateProgressText();
		// 音量提示面板的定时隐藏
		if (volumeHintTimeout.value == null) {
			// 当定时为null，直接跳过
		} else if (volumeHintTimeout.value === 0) {
			isShowVolumeHint.value = false;
			isCursorStatic.value = false;
			volumeHintTimeout.value = null as unknown as number;
		} else if (volumeHintTimeout.value > 0) {
			volumeHintTimeout.value -= 1000;
		} else {
			volumeHintTimeout.value = 0;
		}
		// 总控制面板的定时隐藏
		if (controlsHintTimeout == null || currentTime.value === 0 || currentTime === duration) {
			// 当定时为null、视频还未播放或播放完毕的时候，直接跳过
		} else if (controlsHintTimeout.value === 0) {
			isCursorStatic.value = true;
			controlsHintTimeout.value = null as unknown as number;
		} else if (controlsHintTimeout.value > 0) {
			controlsHintTimeout.value -= 1000;
		} else {
			controlsHintTimeout.value = 0;
		}
		// 根据视频的readyState判断下一帧是否已加载，并控制loading的显示
		isShowLoading.value = videoDom.value!.readyState < 3;
	}.bind(this);
	intervals.value.push(Number(setInterval(interval, 1000)));
	let mouseMove: PlayerEvent = {
		eventName: "mousemove",
		element: videoDom.value,
		method: function () {
			isCursorStatic.value = false;
			controlsHintTimeout.value = 2000;
		}.bind(this)
	}; // 视频dom监听器，用于控制鼠标的显示
	let fullscreenchange: PlayerEvent = {
	...
	let onprogress: PlayerEvent = {
		eventName: "progress",
		element: videoDom.value,
		method: function () {
			buffered.value = videoDom.value!.buffered;
		}.bind(this)
	}; // 视频加载下载过程中触发
	let onDurationchange: PlayerEvent = {
		eventName: "durationchange",
		element: videoDom.value,
		method: function (e: MouseEvent) {
			if (!(e.target instanceof HTMLVideoElement)) {
				throw new Error("onDurationchange: event target not an HTMLVideoElement");
			}
			duration.value = e.target!.duration;
		}.bind(this)
	}; // 视频音频时长发生变化时触发
	listeners.value.push(mouseMove);
	listeners.value.push(fullscreenchange);
	listeners.value.push(pic);
	listeners.value.push(onprogress);
	listeners.value.push(onDurationchange);
	videoDom.value.addEventListener("mousemove", mouseMove.method, false);
	window.addEventListener("fullscreenchange", fullscreenchange.method, false);
	videoDom.value.addEventListener("leavepictureinpicture", pic.method, false);
	videoDom.value.addEventListener("progress", onprogress.method, false);
	videoDom.value.addEventListener("durationchange", onDurationchange.method, false);
});
					...
					<div class="player-controls-bottom-right">
						<div class="player-controls-btn" v-if="biBarrageXml != null" @click="toggleShowBarrage">
							<div class="cursor-pointer">
								<svg
									v-show="isShowBarrage"
									class="player-controls-icon"
									viewBox="0 -25 1024 1024"
									version="1.1"
									xmlns="http://www.w3.org/2000/svg"
									p-id="3405"
									width="24"
								...
								<div class="player-controls-btn-hint">{{ isShowBarrage ? "关闭弹幕" : "开启弹幕" }}</div>
							</div>
						</div>
						...
						<div class="player-controls-btn btn-volume">
							<div class="cursor-pointer" @click="toggleMute">
								...
								>
									<path
										d="M128 426.666667v170.666666c0 23.466667 19.2 42.666667 42.666667 42.666667h128l140.373333 140.373333c26.88 26.88 72.96 7.68 72.96-30.293333V273.493333c0-37.973333-46.08-57.173333-72.96-30.293333L298.666667 384H170.666667c-23.466667 0-42.666667 19.2-42.666667 42.666667z m576 85.333333A192 192 0 0 0 597.333333 340.053333v343.466667c63.146667-31.146667 106.666667-96 106.666667-171.52zM597.333333 189.866667v8.533333c0 16.213333 10.666667 30.293333 25.6 36.266667C733.013333 278.613333 810.666667 386.56 810.666667 512s-77.653333 233.386667-187.733334 277.333333c-15.36 5.973333-25.6 20.053333-25.6 36.266667v8.533333c0 26.88 26.88 45.653333 51.626667 36.266667C793.6 815.36 896 675.84 896 512s-102.4-303.36-247.04-358.4c-24.746667-9.813333-51.626667 9.386667-51.626667 36.266667z"
										p-id="5504"
										fill="#ffffff"
									></path>
								</svg>
								<svg
									v-else-if="volumePercent == 0"
									t="1655365118782"
									class="player-controls-icon"
									viewBox="0 0 1024 1024"
									version="1.1"
								...
								>
									<path
										fill="#ffffff"
										d="m141.04361,426.66667l0,170.66666c0,23.46667 19.2,42.66667 42.66667,42.66667l128,0l140.37333,140.37333c26.88,26.88 72.96,7.68 72.96,-30.29333l0,-476.58667c0,-37.97333 -46.08,-57.17333 -72.96,-30.29333l-140.37333,140.8l-128,0c-23.46667,0 -42.66667,19.2 -42.66667,42.66667zm576,85.33333a192,192 0 0 0 -106.66667,-171.94667l0,343.46667c63.14667,-31.14667 106.66667,-96 106.66667,-171.52z"
									/>
									<g stroke="null">
										<g
											stroke="null"
											transform="matrix(0.023577325882169053,0,0,0.023577325882169053,-2.5802111352677457,-0.4227954069756379) "
										>
											<rect stroke="null" x="20565.485009" y="20471.25898" width="582" height="402" fill="none" />
										</g>
										<g
											stroke="null"
											transform="matrix(1.0006136631769564,0,0,1.0006136631769564,-20566.151143688643,-20575.30244282175) "
										>
											<path
												stroke="null"
												d="m20694.485009,21160.135034zm469.33333,-407.46666l0,8.53333c0,16.21333 10.66667,30.29333 25.6,36.26667c110.08,43.94666 187.73334,151.89333 187.73334,277.33333s-77.65334,233.38667 -187.73334,277.33333c-15.36,5.97334 -25.6,20.05334 -25.6,36.26667l0,8.53333c0,26.88 26.88,45.65334 51.62667,36.26667c144.64,-55.04 247.04,-194.56 247.04,-358.4s-102.4,-303.36 -247.04,-358.4c-24.74667,-9.81333 -51.62667,9.38667 -51.62667,36.26667z"
												fill-opacity="0.5"
												fill="#ffffff"
											/>
										</g>
							...
							<div class="volume-control">
								<div class="volume-control-wrap">
									...
									<VolumeBar
										...
										@update-volume="updateVolumeByClickBar"
										width="4px"
									></VolumeBar>
								</div>
							</div>
						...
						<div v-if="pictureInPictureEnabled" class="player-controls-btn cursor-pointer" @click="togglePictureInPicture">
							...
							>
								<path
									d="m768,213.33333l-512,0a85.33333,85.33333 0 0 0 -85.33333,85.33334l0,426.66666a85.33333,85.33333 0 0 0 85.33333,85.33334l170.66667,0a42.66667,42.66667 0 1 1 0,85.33333l-170.66667,0a170.66667,170.66667 0 0 1 -170.66667,-170.66667l0,-426.66666a170.66667,170.66667 0 0 1 170.66667,-170.66667l512,0a170.66667,170.66667 0 0 1 170.66667,170.66667l0,128a42.66667,42.66667 0 1 1 -85.33334,0l0,-128a85.33333,85.33333 0 0 0 -85.33333,-85.33334zm-128,341.33334a128,128 0 0 0 -128,128l0,85.33333a128,128 0 0 0 128,128l170.66667,0a128,128 0 0 0 128,-128l0,-85.33333a128,128 0 0 0 -128,-128l-170.66667,0z"
								/>
								<g stroke="null">
									<g
										stroke="null"
										transform="matrix(0.6896517266997474,0,0,0.6896517266997474,-10241.200782450309,-10001.206060939305) "
									>
										<rect stroke="null" x="15122.523407" y="14826.656681" width="582" height="402" fill="none" />
									</g>
									<g
										stroke="null"
										transform="matrix(0.6896517266997474,0,0,0.6896517266997474,-10241.200782450309,-10001.206060939305) "
									>
										<path
											stroke="null"
											d="m15503.523407,14924.856681l-161.8,0c-66.2,0 -120,53.8 -120,120l0,161.8c0,22.1 17.9,40 40,40s40,-17.9 40,-40l0,-144.4l169.8,169.8c7.8,7.8 18,11.7 28.3,11.7c10.2,0 20.5,-3.9 28.3,-11.7c15.6,-15.6 15.6,-40.9 0,-56.6l-170.7,-170.6l146.1,0c22.1,0 40,-17.9 40,-40s-17.9,-40 -40,-40z"
										/>
									</g>
								</g>
							</svg>
							<div class="player-controls-btn-hint">{{ isPictureInPicture ? "关闭画中画" : "开启画中画" }}</div>
						</div>
						<div class="player-controls-btn cursor-pointer" @click="togglePageFullscreen">
							...
							<div class="player-controls-btn-hint">{{ isPageFullscreen ? "退出网页全屏" : "网页全屏" }}</div>
						</div>
							...
							>
								...
								<g>
									...
									<g stroke="null">
										<g
											stroke="null"
											transform="matrix(0.9957858808448392,0,0,0.9957858808448392,-3349.1231593465923,-3349.1463172975646) "
										>
										...
										>
											<path
												stroke="null"
												d="m3077.012198,3971.103436zm588.6,1l-161.8,0c-22.1,0 -40,17.9 -40,40s17.9,40 40,40l144.4,0l-169.8,169.8c-15.6,15.6 -15.6,40.9 0,56.6c7.8,7.8 18,11.7 28.3,11.7c10.2,0 20.5,-3.9 28.3,-11.7l170.7,-170.7l0,146.1c0,22.1 17.9,40 40,40s40,-17.9 40,-40l0,-161.8c-0.1,-66.2 -53.9,-120 -120.1,-120z"
											/>
											<path
												stroke="null"
												d="m3077.012198,3971.103436zm588.6,1l-161.8,0c-22.1,0 -40,17.9 -40,40s17.9,40 40,40l144.4,0l-169.8,169.8c-15.6,15.6 -15.6,40.9 0,56.6c7.8,7.8 18,11.7 28.3,11.7c10.2,0 20.5,-3.9 28.3,-11.7l170.7,-170.7l0,146.1c0,22.1 17.9,40 40,40s40,-17.9 40,-40l0,-161.8c-0.1,-66.2 -53.9,-120 -120.1,-120z"
									...
									<g stroke="null">
										<g
											stroke="null"
											transform="matrix(0.9926195208669554,0,0,0.9926195208669554,-15173.84311031432,-15173.842709950044) "
										>
										...
										>
											<path
												stroke="null"
												d="m16017.720739,14877.291967zm160.8,748.6l-144.4,0l169.8,-169.8c15.6,-15.6 15.6,-40.9 0,-56.6c-15.6,-15.6 -40.9,-15.6 -56.6,0l-170.6,170.7l0,-146.1c0,-22.1 -17.9,-40 -40,-40s-40,17.9 -40,40l0,161.9c0,66.2 53.8,120 120,120l161.8,0c22.1,0 40,-17.9 40,-40s-17.9,-40.1 -40,-40.1z"
											/>
										</g>
									</g>
								</g>
							</svg>
							<div class="player-controls-btn-hint btn-fullscreen-hint">{{ isFullscreen ? "退出全屏" : "进入全屏" }}</div>
						</div>
					</div>
				</div>
			</div>
...
.player-paused-state {
	...
	border-radius: 8px;
	box-shadow: 1px 1px 5px rgb(34 34 34 / 50%);
...
.player-controls {
	...
	padding-top: 12px;
	visibility: visible;
	background-image: linear-gradient(rgb(0 0 0 / 0%), rgb(0 0 0 / 40%), rgb(0 0 0 / 60%));
	opacity: 1;
